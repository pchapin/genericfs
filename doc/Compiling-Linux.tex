...
\author{\copyright\ Copyright 2015 by Peter C. Chapin}
\date{Last Revised: August 6, 2015}
\maketitle
...
virtual machines are in the companion document \textit{DevBox and HackBox}.
...
This document describes the build process specifically for kernel version 3.18.14 on 64~bit
Ubuntu Linux 14.04 (trusty). The commands below reflect this version number and platform. If
...
3.18.14, the name of this archive is \filename{linux-3.18.14.tar.xz}. The last component of the
version number is a release level. As 3.18 matures, it advances through several releases.

Once you download the tarball you will need to unpack it somewhere on your development system.

\begin{commands}
\$ unxz < linux-3.18.14.tar.xz | tar xf -
\end{commands}

Unpacking the tarball will create a \filename{linux-3.18.14} directory beneath the current
directory. While doing kernel development it is probably best to unpack the kernel in your home
directory. That will make it easy for you to do development under your normal user account. You
will need to be root to install the kernel, but you can build it as an ordinary user.
...
14.04 distribution stores a copy of the current configuration in
\filename{/boot/config-3.16.0-45-generic} where 45 is the current (as of the time of this
...
system:
\begin{commands}
\$ cp /boot/config-3.16.0-45-generic .config \\
\$ make oldconfig
\end{commands}
...
configuration of the development system, you should instead take it from the experimental
system.
...
module). In general you can just hit `enter' to accept the default in most cases.\footnote{You
...
kernel with debug info'' turned on (this is the default in the Ubuntu 14.04 configuration). Be
aware that this option greatly increases the amount of disk space required to build the kernel
since every object file produced by the compiler contains debugging symbols.\footnote{Several
  gigabytes of disk space are required for a full build using debugging.} This option is
appropriate if you are creating a User Mode Linux kernel (see Section~\ref{sec:UML}) or if you
plan to use a kernel debugger or a tool to analyze kernel crash dumps.

If you are building a kernel for use on an experimental system, and you wish to debug it
remotely using KGDB, you will want to enable KGDB support in the kernel configuration (this is
also the default in the Ubuntu 14.04 configuration). I say more about setting up and using KGDB
in the companion document \textit{DevBox and HackBox}.
...
(as modules) even though your system will likely never use them. Nevertheless you may find some
drivers useful, especially in an experimental context, so it doesn't hurt to build them all.
...
\begin{commands}
\# cd /lib/modules \\
\# tar cf - 3.18.4 | gzip > modules-3.18.14.tar.gz
\end{commands}

Then transfer \filename{modules-3.18.14.tar.gz} to the experimental system and unpack it there
using:
\begin{commands}
\# cd /lib/modules \\
\# gunzip < modules-3.18.14.tar.gz | tar xf -
\end{commands}
...
\begin{commands}
\# cp arch/x86/boot/bzImage /boot/vmlinuz-3.18.14 \\
\# cp .config /boot/config-3.18.14 \\
\# cp System.map /boot/System.map-3.18.14
\end{commands}
...
command is:
\begin{commands}
\# mkinitramfs -o /boot/initrd.img-3.18.14 3.18.14
\end{commands}
...
Once you have \filename{vmlinuz-3.18.14} and \filename{initrd.img-3.18.14} in the
...
LNX=/home/peter/Linux/linux-3.18.14
