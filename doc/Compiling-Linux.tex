...
This document describes the build process specifically for kernel version 4.12.8 on 64~bit Ubuntu
Linux 16.04. The commands below reflect this version number and platform. If you are using a
...
them. Consult the documentation for your distribution for more information.

The kernel source is kept in a compressed archive called a \newterm{tarball}. For version
4.12.8, the name of this archive is \filename{linux-4.12.8.tar.xz}. The last component of the
version number is a release level. As 4.12 matures, it advances through several releases.

Once you download the tarball you will need to unpack it somewhere on your development system.

\begin{commands}
\$ unxz < linux-4.12.8.tar.xz | tar xf -
\end{commands}

Unpacking the tarball will create a \filename{linux-4.12.8} directory beneath the current
...
will need to be root to install the kernel, but you can configure and build it as an ordinary
user.
...
16.04 distribution stores a copy of the current configuration in
\filename{/boot/config-4.4.0-92-generic} where 92 is the current (as of the time of this
...
system:
\begin{commands}
\$ cp /boot/config-4.4.0-92-generic .config \\
\$ make oldconfig
\end{commands}
...
kernel with debug info'' turned on (this is the default in the Ubuntu 16.04 configuration). Be
...
plan to use a kernel debugger (such as KGDB) or a tool to analyze kernel crash dumps.
...
also the default in the Ubuntu 16.04 configuration). I say more about setting up and using KGDB
