...
are build into the kernel image during the build process. For our purposes we don't need to
provide such a file, but you'll need to adjust the configuration to say so. Search the
\filename{.config} file for the following three configuration parameters:
...
information is the default in the Ubuntu 24.04 configuration. However, be aware that this option
greatly increases the amount of disk space required to build the kernel since every object file
produced by the compiler contains debugging symbols.\footnote{Several gigabytes of disk space
are required for a full build using debugging.}
