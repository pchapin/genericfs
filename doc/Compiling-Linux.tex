...
Then transfer \filename{modules-6.9.3.tar.gz} to the experimental system and unpack it there
using:
\begin{commands}
  \# cd /lib/modules \\
...
\filename{/boot} directory on the target system. For example, using paths relative to the root
of the kernel source tree, you might do the following:
\begin{commands}
...
If you prepared a kernel for a separate experimental system you should transfer the files to
that system and install them in the \filename{/boot} directory there, as the commands above
suggest.
