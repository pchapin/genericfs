 ...
 *
 * \brief This file contains the inode operations methods for GenericFS.
 */
...
struct inode *gfs_iget( struct super_block *sb, unsigned long ino )
{
    ...
    if(S_ISREG( inode->i_mode ) ) {
        ...
        // The last parameter here is the "raw" device number. GenericFS doesn't currently
        // support devices so this has to be fixed later when such support is added. (The zero
        // is there so that the code will compile.
