 ...
 *
 * \brief This file contains the functions to initialize and clean up the module as well as the
 * definitions of the super block methods. This file is, in effect, the "main" file of the
 * module.
 */
...
static
struct dentry *gfs_mount( struct file_system_type *fs_type, int flags, const char *dev_name, void *data);
...
{
    struct gfs_ssupplementary *super_sup = (struct gfs_ssupplementary *)sb->s_fs_info;
...
// Stan Brinkerhoff (Fall 2003)
// 
// Originally: http://www-db.stanford.edu/~manku/bitcount/bitcount.html
// 
static int gfs_bitcount( unsigned int n )
{  
    int count = 0;    
    while( n ) {
        count += n & 0x1u;
...
// This function is due to Stan Brinkerhoff (Fall 2003).
static int gfs_freespace( struct super_block *sb )
{
    int x;
    int m;
    int counter = 0;
...
// This function is due to Stan Brinkerhoff (Fall 2003).
static int gfs_inodefreespace( struct super_block *sb )
{
    int x;
    int m;
    int counter = 0;
...
// This method is due to Stan Brinkerhoff (Fall 2003).
int gfs_statfs( struct dentry *de, struct kstatfs *buf )
{
    struct gfs_ssupplementary *super_sup = (struct gfs_ssupplementary *)de->d_sb->s_fs_info;
...
int gfs_fill_super( struct super_block *sb, void *data, int flag )
{
    ...
    super_sup->gsb = (struct gfs_super_block *)( super_sup->super_buffer->b_data );
    ...
    // fields as well. If the data in the position of the generic magic number just happened to
    // be 0xDEADBEEF but the system was otherwise nonsense... we don't want to try using it.
...
static struct dentry *gfs_mount(
