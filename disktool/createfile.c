...
void create_file( int fd )
{
    ...
    lseek( fd, ( 1 + 2 * freemap_blocksize) * BLOCKSIZE + sizeof(struct gfs_inode) * parent_inode, SEEK_SET );
    ...
    *( offset + 8 ) = strlen( name );
    ...
    lseek( fd, (1 + 2*freemap_blocksize ) * BLOCKSIZE + sizeof(struct gfs_inode) * current_inode, SEEK_SET);
