...
static int menu( void )
{
    ...
    clear( );
    mvprintw( 2, 1, "Partition  : %u bytes (%u blocks)", block_count * BLOCKSIZE, block_count );
    mvprintw( 3, 1, "Free Map   : %u bytes (%u blocks)", freemap_bytesize, freemap_blocksize );
    mvprintw( 4, 1, "Inode Table: %u bytes (%u blocks)", inodetable_bytesize, inodetable_blocksize );
    available_blocks = block_count - 2*freemap_blocksize - inodetable_blocksize;
    mvprintw( 5, 1,  "Available  : %u bytes (%u blocks)", available_blocks * BLOCKSIZE, available_blocks );
